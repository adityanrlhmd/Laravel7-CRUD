        ...
        <div class="container-fluid">
            <div class="card " style="border: none">
                <div class="button p-3">
                    <a class="btn btn-primary float-right my-auto" href="{{ route('fitur.create') }}">Create</a>
                </div>
                <div class="container">
                        ...
                        <tbody style="color: black; ">
                            @forelse ($items as $no => $item)
                                <tr>
                                    ...
                                    <td>{{ $item->text }}</td>
                                    <td>
                                        <a href="{{ route('fitur.edit', $item->id) }}"
                                            class="btn btn-outline-warning">Edit</a>
                                        <a onclick="return confirm('Apakah Anda yakin untuk mengahpus ini?')">
                                            <form action="{{ route('fitur.destroy', $item->id) }}" method="post"
                                                class="d-inline">
                                                @csrf
                                                @method('delete')
